...
def generate_python_module(path: Path) -> tuple[str, Module]:
    ...
    names = ",\n    ".join(name for name in dir(python_module) if not name.startswith("_"))
...
def beet_default(ctx: Context):
    ...
    for dir in src.iterdir():
        ...
        if dir.is_dir():
            ...
            raise ValueError(f"{dir} is not a directory. Please place all modules within their own directory.")
